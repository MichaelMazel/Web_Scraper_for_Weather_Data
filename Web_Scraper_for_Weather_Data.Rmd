...
# My token to use the API is hidden but is accessed with this code
load_dot_env()
TOKEN = Sys.getenv("token") # insert your token here
```
...
weather$date_time <- force_tzs(ymd_hms(weather$date_time), tzones = "UTC", tzone_out = "America/New_York") # due to this conversion, a few hours at the end of each month will not be scraped
...
# Below is cloud coverage estimates based off research
...
We will create a new column for precipitation. We will convert to the approximate precipitation rate in mm/hr. This may not be exactly what was measured from the API, but it is the best guess based off other sources
```{r}
# EDIT TO REMOVE NULL VALUES
